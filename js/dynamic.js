(function () {
    ...
    // Phone Links
    setHref("[data-phone-link]", "tel:" + config.phone.replace(/\s+/g, ""));
    ...
    if (config.googleScript && config.googleScript.contactForm) {
        setAttr("form[data-contact-form]", "action", config.googleScript.contactForm);
    }
    ...
    if (titleEl && titleEl.dataset.pageTitle) {
        document.title = titleEl.dataset.pageTitle + " | " + config.siteName;
        titleEl.textContent = document.title;
    }

    const descriptionEl = document.querySelector("[data-site-description]");
    if (descriptionEl && descriptionEl.dataset.pageDescription) {
        const description = descriptionEl.dataset.pageDescription;
